...
from app.database.models import async_session, TestPeriod, User, Subscribers, Payments
...
# ====== Клиенты на сервере WG: просто текст ======
...
async def clients_on_server_wg(message: Message):
    ...
    await message.answer(f"Количество клиентов на сервере WG: {result}", reply_markup=kb.stat_kb)
